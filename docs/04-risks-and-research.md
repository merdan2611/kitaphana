...
| **R4** | From inside Turkmenistan: does a foreign-registrar domain resolve, is the droplet's Frankfurt address reachable at all over ordinary mobile and home connections, and how fast is an 80 MB download? | The server is now abroad ([ADR-0019](adr/0019-digitalocean-droplet-hosting.md)). If its address range is filtered, readers cannot reach the library, and the choice is another region, another provider, or back to a local server. If it is merely slow, that is the accepted cost, but it should be a measured number. Port 80 is open on a droplet, so Let's Encrypt's HTTP challenge works. **Blocks Sprint 02.** | Sprint 02 |
...
| **The server is abroad** | Readers in Turkmenistan get slower pages and downloads than a local server would give them, or, if the address is filtered, cannot reach the library at all. | Measure it first thing in Sprint 02 (R4). A droplet snapshot can be restored in another region in minutes, and nothing in the stack is tied to DigitalOcean, so moving to a local server later is replaying the Sprint 02 notes — [ADR-0019](adr/0019-digitalocean-droplet-hosting.md). |
